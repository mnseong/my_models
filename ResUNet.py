...
# code from minseong-kweon
def load_module(module, cls):
    m = importlib.import_module(module)
    return getattr(m, cls)


class ResUNet(nn.Module):
    def __init__(self, encoder='resnet50', pretrained=True, fusion_out_ch=128):

        super(ResUNet, self).__init__()
        assert encoder in ['resnet18', 'resnet34', 'resnet50', 'resnet101', 'resnet152'], "Plz Check your Backbone!!"
        if encoder in ['resnet18', 'resnet34']:
            filters = [64, 128, 256, 512]
        else:
            filters = [256, 512, 1024, 2048]
        resnet = load_module("torchvision.models", encoder)(pretrained=pretrained)
